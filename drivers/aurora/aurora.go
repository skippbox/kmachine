...
const (
	defaultInstanceProfile  = "agile | 2g"
	defaultDiskSize         = 50
	defaultImage            = "ubuntu-14.04"
	defaultAvailabilityZone = "zone01.ams02"
)
...
func (d *Driver) GetCreateFlags() []mcnflag.Flag {
	return []mcnflag.Flag{
		mcnflag.StringFlag{
			EnvVar: "AURORA_ENDPOINT",
			Name:   "aurora-url",
			Usage:  "aurora API endpoint",
		},
		mcnflag.StringFlag{
			EnvVar: "AURORA_API_KEY",
			Name:   "aurora-api-key",
			Usage:  "aurora API key",
		},
		mcnflag.StringFlag{
			EnvVar: "AURORA_API_SECRET",
			Name:   "aurora-api-secret-key",
			Usage:  "aurora API secret key",
		},
		mcnflag.StringFlag{
			EnvVar: "AURORA_INSTANCE_PROFILE",
			Name:   "aurora-instance-profile",
			Value:  defaultInstanceProfile,
			Usage:  "aurora instance profile (small, medium, large, ...)",
		},
		mcnflag.IntFlag{
			EnvVar: "AURORA_DISK_SIZE",
			Name:   "aurora-disk-size",
			Value:  defaultDiskSize,
			Usage:  "aurora disk size (10, 50, 100, 200, 400)",
		},
		mcnflag.StringFlag{
			EnvVar: "AURORA_IMAGE",
			Name:   "aurora-image",
			Value:  defaultImage,
			Usage:  "aurora image template",
		},
		mcnflag.StringSliceFlag{
			EnvVar: "AURORA_SECURITY_GROUP",
			Name:   "aurora-security-group",
			Value:  []string{},
			Usage:  "aurora security group",
		},
		mcnflag.StringFlag{
			EnvVar: "AURORA_AVAILABILITY_ZONE",
			Name:   "aurora-availability-zone",
			Value:  defaultAvailabilityZone,
			Usage:  "aurora availibility zone",
		},
	}
}
...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	d.URL = flags.String("aurora-endpoint")
	d.ApiKey = flags.String("aurora-api-key")
	d.ApiSecretKey = flags.String("aurora-api-secret-key")
	d.InstanceProfile = flags.String("aurora-instance-profile")
	d.DiskSize = flags.Int("aurora-disk-size")
	d.Image = flags.String("aurora-image")
	securityGroups := flags.StringSlice("aurora-security-group")
	if len(securityGroups) == 0 {
		securityGroups = []string{"docker-machine"}
	}
	d.SecurityGroup = strings.Join(securityGroups, ",")
	d.AvailabilityZone = flags.String("aurora-availability-zone")
	d.SwarmMaster = flags.Bool("swarm-master")
	d.SwarmHost = flags.String("swarm-host")
	d.SwarmDiscovery = flags.String("swarm-discovery")

	if d.URL == "" {
		d.URL = "https://api.auroracompute.eu/ams"
	}
	if d.ApiKey == "" || d.ApiSecretKey == "" {
		return fmt.Errorf("Please specify an API key (--aurora-api-key) and an API secret key (--aurora-api-secret-key).")
	}
...
func (d *Driver) Create() error {
	log.Infof("Querying aurora for the requested parameters...")
	client := egoscale.NewClient(d.URL, d.ApiKey, d.ApiSecretKey)

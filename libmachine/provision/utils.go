...
func installk8sGeneric(p Provisioner) error {
	...
	if _, err := p.SSHCommand(fmt.Sprintf("sudo docker run -d --net=host --pid=host --privileged --restart=always --name head -v /sys:/sys:ro -v /var/run:/var/run:rw -v /:/rootfs:ro -v /dev:/dev -v /var/lib/docker/:/var/lib/docker:ro -v /var/lib/kubelet/:/var/lib/kubelet:rw -v /tmp/kube.conf:/etc/kubernetes/kubelet.kubeconfig -v /tmp/master.json:/etc/kubernetes/manifests/master.json -v /var/run/kubernetes/tokenfile.txt:/tmp/tokenfile.txt -v /tmp/policy.jsonl:/etc/kubernetes/policies/policy.jsonl gcr.io/google_containers/hyperkube:v1.1.4 /hyperkube kubelet --cluster-dns=10.0.0.10 --cluster-domain=cluster.local --containerized --allow-privileged=true --api_servers=http://localhost:8080 --v=2 --address=0.0.0.0 --enable_server --hostname_override=127.0.0.1 --config=/etc/kubernetes/manifests --kubeconfig=/etc/kubernetes/kubelet.kubeconfig")); err != nil {
